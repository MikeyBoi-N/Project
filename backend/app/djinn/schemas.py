...
class Image(ImageBase):
    ...
    id: uuid.UUID = Field(
        ..., description="Unique identifier for the image node in Neo4j"
    )
    storage_uri: str = Field(
        ..., description="URI/identifier for the image in Object Storage"
    )
...
class DetectedObjectBase(BaseModel):
    ...
    object_class: str = Field(
        ...,
        description="Classification label of the detected object (e.g., 'car', 'person')",
    )
    confidence: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Confidence score from the detection model (0.0 to 1.0)",
    )
    bounding_box: Optional[List[Tuple[float, float]]] = Field(
        None,
        description="Coordinates of the bounding box within the image (e.g., [[xmin, ymin], [xmax, ymax]] or polygon)",
    )
    # CRUCIAL for map display:
    latitude: Optional[float] = Field(
        None, description="Estimated latitude of the detected object"
    )
    longitude: Optional[float] = Field(
        None, description="Estimated longitude of the detected object"
    )
...
class DetectedObjectCreate(DetectedObjectBase):
    ...
    image_id: uuid.UUID = Field(
        ..., description="ID of the source image where the object was detected"
    )
...
class DetectedObject(DetectedObjectBase):
    ...
    id: uuid.UUID = Field(
        ..., description="Unique identifier for the detected object node in Neo4j"
    )
    image_id: uuid.UUID = Field(..., description="ID of the source image")
    created_at: datetime = Field(
        ..., description="Timestamp when the detection was recorded"
    )
