...
class Document(DocumentBase):
    ...
    id: uuid.UUID = Field(..., description="Unique identifier for the document node in Neo4j")
    storage_uri: str = Field(..., description="URI or identifier for the document in Object Storage (e.g., MinIO path/key)")
    created_at: datetime = Field(..., description="Timestamp when the document was ingested")
    updated_at: datetime = Field(..., description="Timestamp when the document metadata was last updated")
...
class DocumentSearchResult(BaseModel):
    ...
    results: List[Document] = Field(..., description="List of documents matching the search query")
    total_count: int = Field(..., description="Total number of matching documents found")