...
from ..db.session import get_driver # Placeholder, might need a specific dependency injector
...
async def upload_document(
    ...
    """
    logger.info(f"Received document upload request: {file.filename} by user {current_user.email}")
    ...
    try:
        ...
        logger.error(f"Failed to upload document {file.filename} to storage: {e}", exc_info=True)
        raise HTTPException(
    ...
    try:
        ...
        logger.info(f"Document metadata created for: {created_document.filename} with ID: {created_document.id}")
        return created_document
    except Exception as e:
        logger.error(f"Failed to create document metadata for {file.filename}: {e}", exc_info=True)
...
async def search_documents(
    ...
    """
    logger.info(f"Received search request: '{search_input.query}' by user {current_user.email}")
    ...
    try:
        ...
        logger.debug(f"Search returned {len(search_results)} documents for query: '{search_input.query}'")
        return schemas.DocumentSearchResult(results=search_results, total_count=total_count)

    except Exception as e:
        logger.error(f"Failed to search documents for query '{search_input.query}': {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
