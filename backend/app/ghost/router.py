...
@router.post("/signals/ingest", response_model=schemas.SignalEvent, status_code=status.HTTP_201_CREATED)
async def ingest_signal_event(
    ...
    recording_file: Optional[UploadFile] = File(None, description="Optional signal recording file"),
    ...
    logger.debug(f"Signal metadata received: {signal_data.model_dump_json(indent=2)}") # Pydantic V2
    ...
    if recording_file:
        ...
        try:
            ...
            recording_storage_uri = f"minio:ghost-recordings/{uuid.uuid4()}_{recording_file.filename}"
            logger.debug(f"Placeholder recording storage URI generated: {recording_storage_uri}")
            ...
            logger.error(f"Failed to upload recording {recording_file.filename} to storage: {e}", exc_info=True)
            raise HTTPException(
    ...
    try:
        ...
        logger.info(f"Signal event metadata placeholder created with ID: {created_event.id}")
...
async def get_signals( # Correctly indented function definition
    ...
    """
    logger.info(f"Request received to retrieve signal events (skip={skip}, limit={limit})")
    try:
        # Use the imported crud module
        signal_events = await crud.get_signal_events(driver=db_driver, skip=skip, limit=limit)
