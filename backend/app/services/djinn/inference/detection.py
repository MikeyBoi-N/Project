    ...
    try:
        ...
        if results and results[0].boxes and results[0].boxes.data is not None:
            ...
            for box_data in results[0].boxes.data:
                ...
                class_name = model.names[int(cls)] if model.names else f"class_{int(cls)}"

                detection = {
                    'bbox_pixels': [float(x1), float(y1), float(x2), float(y2)], # Ensure coords are floats
...
if __name__ == '__main__':
    ...
    logger.info("Detection module loaded. Model should be loaded if no errors occurred.")
