...
async def get_driver() -> AsyncDriver:
    ...
    if _driver is None:
        for attempt in range(MAX_RETRIES):
            try:
                logging.info(f"Attempting to connect to Neo4j at {settings.NEO4J_URI} with user {settings.NEO4J_USER} (Attempt {attempt + 1}/{MAX_RETRIES})")
                ...
                logging.warning(f"Neo4j connection attempt {attempt + 1} failed: {e}. Retrying in {RETRY_DELAY_SECONDS} seconds...")
                if attempt + 1 == MAX_RETRIES:
                    logging.error("Max retries reached. Failed to initialize Neo4j driver.")
                    raise RuntimeError("Could not connect to Neo4j database after multiple attempts.") from e
                await asyncio.sleep(RETRY_DELAY_SECONDS)
            except Exception as e: # Catch other potential driver errors
                logging.error(f"An unexpected error occurred during Neo4j driver initialization: {e}")
                raise RuntimeError("Could not connect to Neo4j database due to an unexpected error.") from e

        if _driver is None: # Should be caught by the retry loop exception, but as a safeguard
