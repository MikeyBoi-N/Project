...
class TesseractSceneBase(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="User-provided description of the scene"
    )
    timestamp: datetime = Field(
        ..., description="Timestamp relevant to the scene data (e.g., capture time)"
    )
    ...
    footprint_geojson: Optional[Dict[str, Any]] = Field(
        None,
        description="GeoJSON representation (Polygon or MultiPolygon) of the scene's 2D footprint on the map",
    )

    # Metadata about the reconstruction process or source data
    source_data_description: Optional[str] = Field(
        None, description="Description of the source data used for reconstruction"
    )
    reconstruction_parameters: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Parameters used during reconstruction"
    )
...
class TesseractScene(TesseractSceneBase):
    ...
    id: uuid.UUID = Field(
        ..., description="Unique identifier for the Tesseract scene node in Neo4j"
    )
    scene_data_storage_uri: str = Field(
        ...,
        description="URI/identifier for the main scene data file (e.g., .splat) in Object Storage",
    )
    # Optional: Add URIs for other related files if needed
    created_at: datetime = Field(
        ..., description="Timestamp when the scene was logged into the system"
    )
    updated_at: datetime = Field(
        ..., description="Timestamp when the scene metadata was last updated"
    )
